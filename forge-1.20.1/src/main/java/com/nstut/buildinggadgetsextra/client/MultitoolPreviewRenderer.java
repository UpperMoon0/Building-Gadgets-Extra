...
public final class MultitoolPreviewRenderer {
    ...
    public static void render(RenderLevelStageEvent event, Player player, ItemStack multitool) {
        ...
        if (multitool.hasTag()) view.setTag(multitool.getTag().copy());
        if (mode == MultitoolMode.DESTRUCTION) DestructionRenderer.render(event, player, view);
        else { VBORenderer.buildRender(event, player, view); VBORenderer.drawRender(event, player, view); }
    }
}
