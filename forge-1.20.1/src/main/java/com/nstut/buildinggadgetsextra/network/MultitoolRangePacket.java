...
public final class MultitoolRangePacket {
    ...
    public static boolean apply(ServerPlayer player, int requestedRange) {
        ...
        if (mode != MultitoolMode.BUILD && mode != MultitoolMode.EXCHANGING) return false;

        int range = MultitoolRangePolicy.clamp(requestedRange, ExtraConfig.multitoolMaxRange());
        ...
        player.displayClientMessage(Component.translatable("buildinggadgets2.messages.range_set", range), true);
        return true;
    }
}
