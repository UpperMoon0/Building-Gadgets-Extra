...
public final class NativeStructureBridge {
    ...
    public static byte[] exportStructure(ServerPlayer player, String name) {
        ...
        for (StatePos entry : source) {
            ...
            minY = Math.min(minY, entry.pos.getY());
            minZ = Math.min(minZ, entry.pos.getZ());
            maxX = Math.max(maxX, entry.pos.getX());
            maxY = Math.max(maxY, entry.pos.getY());
        ...
        if (sourceTags != null) {
            for (TagPos tag : sourceTags) tags.put(tag.pos, tag.tag);
        }
        ...
        accessor.buildingGadgetsExtra$getPalettes().add(
                StructurePaletteInvoker.buildingGadgetsExtra$create(blocks));
        accessor.buildingGadgetsExtra$setSize(
                new BlockPos(maxX - minX + 1, maxY - minY + 1, maxZ - minZ + 1));
        template.setAuthor(player.getGameProfile().name());
    ...
    public static void importStructure(ServerPlayer player, String name, byte[] bytes) {
        ...
        if (accessor.buildingGadgetsExtra$getPalettes().isEmpty()
                || template.getSize().getX() <= 0 || template.getSize().getY() <= 0
    ...
    private static BG2Data data(ServerPlayer player) {
        return BG2Data.get(Objects.requireNonNull(player.level().getServer()).overworld());
    }
